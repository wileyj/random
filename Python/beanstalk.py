#!/usr/bin/python
#
#    python beanstalk.py --region "us-west-1" --app "App-dev-1" 
#
#    python beanstalk.py --region "us-west-1" --app "steppers-dev-1" --scaleup
#    python beanstalk.py --region us-east-1 --app App-Dev-2 --scaleup --maxsize 6
#    
#    python beanstalk.py --region "us-west-1" --app "steppers-dev-1" --scaledown
#    python beanstalk.py --region us-east-1 --app App-Dev-2 --scaledown --maxsize 2
#
...
def get_war(region,appname,environment,app):
    ...
    if len(warfile) == 0 and len(env_app_list) < 1:
        ...
        p = "app-root-"+env_short
        if appname.split("-")[0].lower() == "one":
            p = "app-one-root-"+env_short
        if appname.split("-")[0].lower() == "two":
            p = "app-two-root-"+env_short
...
if __name__ == "__main__":
    ...
    parser.add_argument('--app',           metavar='', default="", help='Beanstalk Environment Name ( App-Prod-1 )' )
    ...
    parser.add_argument('--cert',          metavar='', default="prod.local.com", help='SSL Certificate ( prod.local.com )' )
    parser.add_argument('--keyname',       metavar='', default="admin", help='AWS EC2 Keyname ( admin )' )
    parser.add_argument('--instance',      metavar='', default="m4.large", help='EC2 Instance Type ( m3.medium ) ' )
    parser.add_argument('--account',       metavar='', default="xxxxxxxxxxxxxxx", help='AWS Account Number ( xxxxxxxxxxxxxxx ) ' )
    ...
    parser.add_argument('--bucket',        metavar='', default="cloudformation", help="Bucket To store Cloudformation Json ( cloudformation ) " )
