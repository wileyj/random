...
print "$check_type @options\n" if ($debug) ;
  ...
  open ( DF, "df -k $mount | grep -v Filesystem |" ) ;
  while ( my $dfline = <DF> ) {
    chomp ( $dfline ) ;
    print STDERR "$dfline\n" if ($debug) ;
    if ( $dfline =~ /.*\s+$mount$/ ) {
  ...
  open ( MOUNT, "mount | grep 'remote' |" ) ;
  while ( my $mountline = <MOUNT> ) {
#    if ( $mountline =~ /^$remote\s+on\s+$mount\s+.*\((.*)addr/ ) {
if ($mountline =~/$mount\s+on\s+$remote/){
      #my @realoptions = split(/,/, $1) ;
      my @realoptions = split(/ /, $mountline) ;
      my $realopt="$realoptions[3] $realoptions[4]";
      if ( $wantopt{rw} == 1 ) {
        if ( -d "$mount/.nagios" ) {
